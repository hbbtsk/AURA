...
5. [模块详解](#5-模块详解)
   ...
   - 5.2 [配置层: `app/core/config.py`](#52-配置层-appcoreconfigpy)
   - 5.3 [API 层: `app/api/`](#53-api-层-appapi)
   - 5.4 [Prompt 拆解器: `app/core/prompt_decomposer.py`](#54-prompt-拆解器-appcoreprompt_decomposerpy)
   - 5.5 [意图解析器: `app/core/intent_tagger.py`](#55-意图解析器-appcoreintent_taggerpy)
   - 5.6 [记忆管理层: `app/memory/`](#56-记忆管理层-appmemory)
   - 5.7 [数据模型: `app/memory/models.py`](#57-数据模型-appmemorymodelspy)
   - 5.8 [LangGraph 节点: `app/graph/nodes/`](#58-langgraph-节点-appgraphnodes)
   - 5.9 [工具模块: `app/utils/`](#59-工具模块-apputils)
6. [LangGraph 状态机 (v0.8.0)](#6-langgraph-状态机-v080)
7. [Prompt 编译管道](#7-prompt-编译管道)
8. [三层记忆架构](#8-三层记忆架构)
9. [意图感知 RAG (v2)](#9-意图感知-rag-v2)
10. [SSE 流式处理](#10-sse-流式处理)
11. [配置系统](#11-配置系统)
...
| # | 痛点 | 涉及模块 | 解决状态 |
|---|------|----------|----------|
| 1 | **越权输出** — 模型替 user 写台词/行动 | [`CONSTRAINTS`](app/graph/nodes/context_assemble.py) 区块 + [`OUTPUT_SPEC`](app/graph/nodes/context_assemble.py) COT 校验 + RAG 记忆压缩减少 Prompt 噪声 | ✅ 显著缓解 |
| 2 | **文风污染** — 垃圾小说训练痕迹（臀腿腰胸） | Week 2 FormatGuard（预留） | ❌ 待 Week 2 |
| 3 | **文风固化** — 长时间同一模型锁死 | Week 3 ModelDialectCompiler（预留） | ❌ 待 Week 3 |
| 4 | **状态回退** — 怀孕→生完→又怀孕 | [`CURRENT_STATE`](app/graph/nodes/context_assemble.py) 区块 + StateManager（预留 Day 4） | ✅ 部分缓解 |
...
| 8 | **RPG多角色状态记录缺失** — party/NPC 状态变化全丢 | [`SQLiteStore.sync_dialogue_from_tavo()`](app/memory/sqlite_store.py) + RAG 召回多角色记忆 | ✅ 部分缓解 |
| 9 | **重复记忆/冗余信息** — 同一批 NPC 信息反复记录 | RAG 语义召回替代全量注入 → [`FAISSStore.structured_aware_search()`](app/memory/faiss_store.py) Top-5 | ✅ **根本解决** |
| 10 | **长记忆无 RAG，全量注入** — token 浪费 + 注意力稀释 | RAG 主动召回 Top-5 → [`LONG_TERM_MEMORY`](app/graph/nodes/context_assemble.py) 区块 | ✅ **根本解决** |
| 11 | **模型输出太少** — DeepSeek 输出过短 | [`OUTPUT_SPEC`](app/graph/nodes/context_assemble.py) 长度约束 + Week 2 ResponseLengthGuard | ⚠️ 间接缓解 |
| 12 | **模型输出太多** — Gemini 输出过长，推进剧情 | [`CONSTRAINTS`](app/graph/nodes/context_assemble.py) 负向指令 + [`OUTPUT_SPEC`](app/graph/nodes/context_assemble.py) 上限约束 | ⚠️ 间接缓解 |
| 13 | **系统提示词锁不住** — LLM 偏离人设 | Week 3 模型方言编译器（预留） | ❌ 待 Week 3 |
| 14 | **时间维度缺失 → 已完成事件重复生成** | FAISS 时间加权 + `insert_seq` 单调递增 | ⚠️ 部分缓解 |
| 15 | **用户输入意图隐含 → LLM 默认接话而非渲染反应** | [`IntentTagger`](app/core/intent_tagger.py) 解析意图 → [`USER_INTENT_TAG`](app/graph/nodes/context_assemble.py) 导演指令注入 → 意图感知 RAG 召回氛围记忆 | ✅ **显著缓解** |
...
### 4.2 配置层: [`app/core/config.py`](app/core/config.py)
...
### 4.3 API 层: [`app/api/`](app/api/)

v0.8.2 拆分为三个子模块：

| 文件 | 职责 | 行数 |
|------|------|------|
| [`router.py`](app/api/router.py) | Pydantic 模型 + 轻量端点（/models、/health） | ~100 |
| [`completions.py`](app/api/completions.py) | `/chat/completions` 核心入口 + 初始化 + 会话管理 | ~250 |
| [`streaming.py`](app/api/streaming.py) | SSE 模拟器 + 段落切分 + 遗留直接转发逻辑 | ~450 |

#### `chat_completion()` — LangGraph 入口

1. **请求验证**：检查 messages 和 model 字段
2. **Prompt 保存**：保存原始请求到 `prompt_dumps/` 目录
3. **组装 AgentState**：将请求、会话信息、后端配置等注入 [`AgentState`](app/graph/state.py)
4. **调用工作流**：`await aura_workflow.ainvoke(state, config=...)`
5. **响应包装**：从 final_state 提取生成的内容，按 `stream` 参数包装为 SSE 或非流式响应

#### `_build_streaming_response()` — SSE 模拟器

将 LangGraph 节点内已完成的完整内容，按段落+句子粒度切分，模拟 SSE chunk 流式返回给 TAVO。

### 4.4 Prompt 拆解器: [`app/core/prompt_decomposer.py`](app/core/prompt_decomposer.py)
...
| `world_book` | 世界书 | `=====角色卡结束===== ` 之后、`[Start a new Chat]` 之前的内容 |
| `xml_character_cards` | XML 角色卡 | `<charname>...</charname>` 格式 |

### 4.5 意图解析器: [`app/core/intent_tagger.py`](app/core/intent_tagger.py)
...
### 4.6 记忆管理层: [`app/memory/`](app/memory/)

v0.8.2 拆分为四个子模块：

| 文件 | 职责 |
|------|------|
| [`manager.py`](app/memory/manager.py) | Facade 层，委托给底层子模块（~100 行） |
| [`sqlite_store.py`](app/memory/sqlite_store.py) | SQLite 原始对话 + 会话 + 动态状态 + TAVO 同步 |
| [`faiss_store.py`](app/memory/faiss_store.py) | FAISS 向量索引 + 语义检索 + 结构化感知检索 |
| [`embedding.py`](app/memory/embedding.py) | 本地 bge-small-zh-v1.5 embedding 服务 |
| [`summarizer.py`](app/memory/summarizer.py) | Kimi/DeepSeek 驱动的记忆总结 + 结构化提取 |
| [`models.py`](app/memory/models.py) | 数据模型（IntentStructure, IntentResult） |
...
| 方法 | 所在模块 | 说明 |
|------|----------|------|
| `save_dialogue()` | SQLiteStore | 保存单轮对话到 SQLite |
| `sync_dialogue_from_tavo()` | SQLiteStore | 倒序同步 TAVO 对话（处理编辑/撤回） |
| `search()` | FAISSStore | 传统 embedding + 时间加权检索 |
| `structured_aware_search()` | FAISSStore | 意图感知结构化检索（v2 核心） |
| `summarize_and_store()` | MemorySummarizer | Kimi 总结 + 结构化字段提取 + 存入 FAISS |
| `add_memory()` | FAISSStore | 新增单条记忆到 FAISS |
| `import_from_tavo()` | FAISSStore | 从 TAVO System Prompt 导入已有记忆 |
...
### 4.8 LangGraph 节点: [`app/graph/nodes/`](app/graph/nodes/)

v0.8.2 将 [`workflow.py`](app/graph/workflow.py) 拆分为 8 个子模块：

| 文件 | 节点 | 状态 |
|------|------|------|
| [`input_receive.py`](app/graph/nodes/input_receive.py) | InputReceive | ✅ 已实现 |
| [`entity_extract.py`](app/graph/nodes/entity_extract.py) | EntityExtract | 🔲 Mock |
| [`emotion_analyze.py`](app/graph/nodes/emotion_analyze.py) | EmotionAnalyze | 🔲 Mock |
| [`memory_nodes.py`](app/graph/nodes/memory_nodes.py) | MemoryDecision / MemoryRetrieve / MemoryExtract | ✅ 已实现 |
| [`state_style_compiler.py`](app/graph/nodes/state_style_compiler.py) | StateManager / StyleInjection / ModelDialectCompiler | 🔲 Mock |
| [`context_assemble.py`](app/graph/nodes/context_assemble.py) | ContextAssemble | ✅ 已实现 |
| [`llm_quality_output.py`](app/graph/nodes/llm_quality_output.py) | LLMGenerate / FormatGuard / OOCCheck / ContentFilter / OutputReturn | ✅/🔲 |
| [`conditional_edges.py`](app/graph/nodes/conditional_edges.py) | should_retry_after_check | ✅ 已实现 |

### 4.9 工具模块: [`app/utils/`](app/utils/)
...
v0.8.0 引入 **LangGraph** 作为编排层，将原有顺序逻辑拆分为 15 个状态节点。每个节点封装已有业务代码，做到**零侵入原有逻辑**。

### 5.1 架构定位

| 层级 | 职责 | 说明 |
|------|------|------|
| **业务层** | `prompt_decomposer.py` / `intent_tagger.py` / `memory/manager.py` | 保持原样，不感知 LangGraph 存在 |
| **编排层** | `app/graph/workflow.py` | LangGraph StateGraph，15 节点 + 条件边 |
| **入口层** | `app/api/completions.py` | 组装 AgentState → 触发 workflow → 包装响应 |
...
[`AgentState`](app/graph/state.py) 采用 `TypedDict(total=False)`，避免 LangGraph 序列化问题：

| 字段分类 | 关键字段 | 说明 |
|----------|----------|------|
| 输入层 | `request`, `session_id`, `backend` | TAVO 原始请求 + 会话标识 |
| Prompt 编译层 | `raw_prompt`, `decomposed`, `intent_result` | 拆解产物 + 意图解析结果 |
| 记忆层 | `working_memory`, `recent_memories`, `retrieved_memories` | 三层记忆容器 |
| LLM 生成层 | `assembled_prompt`, `llm_response`, `full_content` | 组装后 Prompt + 原始 LLM 返回 |
| 质检层 | `format_check_pass`, `ooc_check_pass`, `content_filter_pass` | 质量控制布尔值 |
| 输出层 | `output_content`, `output_choices` | 最终返回给 TAVO 的内容 |
| 调试层 | `node_debug_info`, `node_execution_log` | 全链路可观测性 |

### 5.4 条件边与重试机制

```python
# ContentFilter → 失败则回退到 ModelDialectCompiler 重新编译
workflow.add_conditional_edges(
    "content_filter",
    should_retry_after_check,
    {
        "output_return": "output_return",      # 通过 → 输出
        "model_dialect_compiler": "model_dialect_compiler",  # 失败 → 重试
    },
)
```

- **最大重试次数**：2 次（由 `retry_count` 字段计数）
- **状态持久化**：`MemorySaver` checkpointer，按 `thread_id`（即 `aura_session_id`）隔离状态

### 5.5 当前节点实现状态

| 节点 | 状态 | 说明 |
|------|------|------|
| `input_receive` | ✅ 已实现 | 解析请求、拆解 Prompt、调用 IntentTagger |
| `memory_retrieve` | ✅ 已实现 | 三层记忆检索 + RAG 召回 |
| `context_assemble` | ✅ 已实现 | 9 区块 System Prompt 组装 |
| `llm_generate` | ✅ 已实现 | 调用 LLM 后端（内部非流式） |
| `output_return` | ✅ 已实现 | 包装响应 + SSE 模拟 |
| `memory_extract` | ✅ 已实现 | 对话同步 + 记忆总结触发 |
| 其余 9 节点 | 🔲 Mock | 节点函数已注册，内部为占位逻辑，待 Day 4-5 填充 |
...
│   │
│   ├── api/                      # API 层（v0.8.3）
│   │   ├── __init__.py           # 统一导出 router, streaming, completions
│   │   ├── router.py             # Pydantic 模型 + APIRouter + /models + /health
│   │   ├── streaming.py          # SSE 流式模拟 + 段落/句子切分 + 非流式/流式请求处理
│   │   └── completions.py        # /chat/completions 入口 + LangGraph 编排调用
│   │
│   ├── graph/                    # LangGraph 编排层（v0.8.0）
│   │   ├── __init__.py
│   │   ├── state.py              # AgentState 定义
│   │   ├── workflow.py           # StateGraph 构建（仅编排，无节点实现）
│   │   └── nodes/                # 15 节点实现（v0.8.3 拆分）
│   │       ├── __init__.py       # 统一导出所有节点
│   │       ├── input_receive.py          # InputReceive: 用户输入接收 + 意图解析
│   │       ├── entity_extract.py         # EntityExtract: 实体识别（预留）
│   │       ├── emotion_analyze.py        # EmotionAnalyze: 情绪分析（预留）
│   │       ├── memory_nodes.py           # MemoryDecision + MemoryRetrieve + MemoryExtract
│   │       ├── state_style_compiler.py   # StateManager + StyleInjection + ModelDialectCompiler
│   │       ├── context_assemble.py       # ContextAssemble: 9 区块 Prompt 编译
│   │       ├── llm_quality_output.py     # LLMGenerate + FormatGuard + OOCCheck + ContentFilter + OutputReturn
│   │       └── conditional_edges.py      # should_retry_after_check: 条件边
│   │
│   ├── memory/                   # 记忆管理层（v0.8.3 拆分）
│   │   ├── __init__.py           # 导出 memory_manager
│   │   ├── manager.py            # 记忆管理 Facade（委托到子模块）
│   │   ├── models.py             # 数据模型（IntentStructure, IntentResult）
│   │   ├── embedding.py          # EmbeddingService（bge-small-zh-v1.5）
│   │   ├── sqlite_store.py       # SQLiteStore（对话/会话/状态/关系图）
│   │   ├── faiss_store.py        # FAISSStore（向量索引 + 结构化感知检索）
│   │   └── summarizer.py         # MemorySummarizer（LLM 驱动摘要 + 结构化提取）
│   │
│   └── utils/                    # 工具模块（v0.8.1）
...
| **v0.8.3** | **2026-05-17** | **大文件拆分：`completions.py`→3 文件，`workflow.py`→8 节点文件，`manager.py`→4 子模块 + Facade** |