...
核心业务逻辑所在，约 800 行。v0.8.0 重构为 **LangGraph 入口**：

#### `chat_completion()` — LangGraph 入口

v0.8.0 以后不再包含顺序业务逻辑，而是作为 **StateGraph 的触发器**：
...
`_handle_streaming_request()` / `_handle_non_streaming_request()` 仍保留在文件中作为降级参考，但新流程不再调用。
...
│   ├── api/
│   │   ├── __init__.py
│   │   └── completions.py        # API 路由（LangGraph 入口）
│   │
