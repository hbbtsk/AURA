...
3. [数据流](#3-数据流)
4. [模块详解](#4-模块详解)
   - 4.1 [入口层: `app/main.py`](#41-入口层-appmainpy)
   - 4.2 [配置层: `app/config.py`](#42-配置层-appconfigpy)
   - 4.3 [API 层: `app/api/completions.py`](#43-api-层-appapicompletionspy)
   - 4.4 [Prompt 拆解器: `app/prompt_decomposer.py`](#44-prompt-拆解器-appprompt_decomposerpy)
   - 4.5 [意图解析器: `app/intent_tagger.py`](#45-意图解析器-appintent_taggerpy)
   - 4.6 [记忆管理层: `app/memory/manager.py`](#46-记忆管理层-appmemorymanagerpy)
   - 4.7 [数据模型: `app/memory/models.py`](#47-数据模型-appmemorymodelspy)
5. [Prompt 编译管道](#5-prompt-编译管道)
6. [三层记忆架构](#6-三层记忆架构)
7. [意图感知 RAG (v2)](#7-意图感知-rag-v2)
8. [SSE 流式处理](#8-sse-流式处理)
9. [配置系统](#9-配置系统)
